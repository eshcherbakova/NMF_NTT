function [A,C,Out] = ntd_fromTT(M,j, origt, coreNway,opts)
...
for k = 1:maxit
    ...
    L(N+1) = 1;
    for i = 1:N 
        L(N+1) = L(N+1)*norm(Asq{i});
    end
    Bsq = Cm; GradC = M;
    for i = 1:N
        Bsq = ttm(Bsq,Asq{i},i);
        GradC = ttm(GradC,i,A{i});
%         disp(size(A{i}));
    end
    GradC = tensor(full(GradC, coreNway));
    ...
    relerr2 = (2*obj)^.5/Mnrm;
%     relerr3 = norm(full(ttensor(C,A))-origt)/orignrm*100;
    % reporting
    Out.hist_obj(k) = obj;
