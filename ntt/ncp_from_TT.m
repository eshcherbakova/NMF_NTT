% Nonnegative Tensor Factorization based on TT (Canonical Decomposition / PARAFAC)
%
...
%
% <Inputs>
%        X : Input data tensor. X is a 'tensor' object of TT-toolbox.
%        In TT format.
%        r : Target low-rank
%
...
function [F,iter,REC]=ncp_from_TT(X,r,varargin)
    rng(100);
    % set parameters
    params = inputParser;
...
function rel_Error = getRelError(X,F_kten,init)
    innerprod_X_F_kten = 0;
    for j = 1:size(F_kten.lambda, 1)
        W = X; 
        for i = 1:ndims(F_kten)
            W = ttm(W, i, F_kten.u{i}(:, j));
        end
        innerprod_X_F_kten = innerprod_X_F_kten + full(W);
    end
    rel_Error = sqrt(max(init.nr_X^2 + norm(F_kten)^2 - 2 * innerprod_X_F_kten,0))/init.nr_X;
end
